  ...
  switch (meta->objType) {
    ...
    case NVDS_OBJECT_TYPE_CUSTOM:
      ...
      if (meta->extMsgSize) {
        ...
        if (dsObj) {
          json_object_set_int_member (jobject, "cc", dsObj->cc);
          json_object_set_int_member (jobject, "camera", dsObj->cam);
        ...
        json_object_set_int_member (jobject, "cc", 0);
        json_object_set_int_member (jobject, "camera", 0);
...
nvds_msg2p_generate (NvDsMsg2pCtx *ctx, NvDsEvent *events, guint size)
{
  ...
  if (ctx->payloadType == NVDS_PAYLOAD_DEEPSTREAM) {
    ...
    message = generate_deepstream_message_custom (ctx, events->metadata);
    if (message) {
      len = strlen (message);
      // Remove '\0' character at the end of string and just copy the content.
      payload->payload = g_memdup (message, len);
