...
#define CONFIG_KEY_LC_NAMES "lc-names"
#define CONFIG_KEY_ROI_NAMES "roi-names"
...
generate_analytics_module_object(NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta, int tipo)
{
  ...
  malesObj = json_object_new();
  femalesObj = json_object_new();

  // generate payload
  if (tipo == 0)
  {
    counts = getString(meta->lc_count, meta->lc_names_size, 0);
    // 0->person   1->car
    meta->fobj_type = 0;
  }
...
// solo se comentaron algunas lineas del codigo original
static gchar *
generate_schema_message(NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  ...
  if (meta->flag_trackingROI)
  {
    JsonObject *trackingObj = json_object_new();
    json_object_set_string_member(trackingObj, "lc_name", to_str(meta->nameLC_tracking));
    json_object_set_object_member(rootObj, "tracking_person", trackingObj);
  }

  if (meta->flag_trackingLC)
  {

    JsonObject *trackingObj = json_object_new();
    json_object_set_string_member(trackingObj, "lc_name", to_str(meta->nameLC_tracking));
    json_object_set_string_member(trackingObj, "gender", to_str(meta->type_gender));
    json_object_set_string_member(trackingObj, "age", to_str(meta->type_age));
    json_object_set_object_member(rootObj, "tracking_person", trackingObj);
  }
...
nvds_msg2p_parse_analytics(NvDsMsg2pCtx *ctx, GKeyFile *key_file, gchar *group)
{
  ...
  for (key = keys; *key; key++)
  {
    ...
    else if (!g_strcmp0(*key, CONFIG_KEY_LC_NAMES))
    {
      keyVal = g_key_file_get_string(key_file, group,
                                     CONFIG_KEY_LC_NAMES, &error);
      analyticsObj.lc = keyVal;
      CHECK_ERROR(error);
    }
    else if (!g_strcmp0(*key, CONFIG_KEY_ROI_NAMES))
    {
      keyVal = g_key_file_get_string(key_file, group,
                                     CONFIG_KEY_ROI_NAMES, &error);
      analyticsObj.roi = keyVal;
      CHECK_ERROR(error);
    }
    else
    {
