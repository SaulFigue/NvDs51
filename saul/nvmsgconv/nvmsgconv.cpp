...
generate_object_object (NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  ...
  switch (meta->objType) {
    ...
    case NVDS_OBJECT_TYPE_CUSTOM:
      ...
      if (meta->extMsgSize) {
        CentroComercialObject *dsObj = (CentroComercialObject *) meta->extMsg;
        if (dsObj) {
          json_object_set_int_member (jobject, "source", dsObj->cc);
          json_object_set_int_member (jobject, "idlc", dsObj->cam);
          json_object_set_int_member (jobject, "in", dsObj->in);
          json_object_set_int_member (jobject, "out", dsObj->out);
        }
      } else {
        // No cc object in meta data. Attach empty person sub object.
        json_object_set_int_member (jobject, "source", 0);
        json_object_set_int_member (jobject, "idlc", 0);
        json_object_set_int_member (jobject, "in", 0);
        json_object_set_int_member (jobject, "out", 0);
      }
      json_object_set_object_member (objectObj, "CC", jobject);
      break;
    default:
...
static JsonObject*
generate_object_cc (NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  // JsonObject *objectObj;
  JsonObject *jobject;
    ...
    case NVDS_OBJECT_TYPE_CUSTOM:
      ...
      if (meta->lcNum > 0) {
        ...
        for(int j=0; j < lcNum; j++){
          if(meta->names[j] != NULL){
            printf("%s | %d\n",meta->names[j],meta->counts[j]);
          }
        }
        if (lcNum > 0) {
          json_object_set_int_member (jobject, "cam", stream_id);
          // puts("LLEGA1-CONVERTER");
          
          for (int i = 0; i < lcNum; ++i) {
            json_object_set_int_member (jobject, meta->names[i], meta->counts[i]);
          }
        }
      } else {
      }
...
static gchar*
generate_deepstream_message_custom (NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  /*
  The JSON structure of the frame
  {
  ...
  */
  ...
  // object object
  // objectObj = generate_object_cc (ctx, meta);

  // root object
  // rootObj = json_object_new ();
  rootObj = generate_object_cc (ctx, meta);
  // json_object_set_object_member (rootObj, "object", objectObj);
...
nvds_msg2p_generate (NvDsMsg2pCtx *ctx, NvDsEvent *events, guint size)
{
  ...
  if (ctx->payloadType == NVDS_PAYLOAD_DEEPSTREAM) {
    message = generate_schema_message (ctx, events->metadata); //Chilenos
    if (message) {
      len = strlen (message);
      // Remove '\0' character at the end of string and just copy the content.
    ...
    message = generate_deepstream_message_custom (ctx, events->metadata); //Stadyn y Saul
    if (message) {
      len = strlen (message);
      // Remove '\0' character at the end of string and just copy the content.
      payload->payload = g_memdup (message, len);
